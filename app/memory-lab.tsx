...
export default function MemoryLab() {
  ...
  async function savePrompt(asNew: boolean) {
    ...
    if (runData?.run.id && !['campus-32-baseline-v1', 'guardian-human-v1', 'guardian-human-v2'].includes(runData.run.id)) {
    ...
    setPromptDraft({ id: prompt.id, name: prompt.name, userPrompt: prompt.userPrompt, agentPrompt: prompt.agentPrompt, guardianSpec: prompt.guardianSpec });
    setConfig((current) => ({ ...current, promptTemplateId: prompt.id, promptName: prompt.name, userPrompt: prompt.userPrompt, agentPrompt: prompt.agentPrompt, guardianSpec: prompt.guardianSpec }));
    setRunData((current) => current && !['campus-32-baseline-v1', 'guardian-human-v1', 'guardian-human-v2'].includes(current.run.id) && current.run.promptTemplateId === prompt.id ? { ...current, run: { ...current.run, promptName: prompt.name, userPrompt: prompt.userPrompt, agentPrompt: prompt.agentPrompt, guardianSpec: prompt.guardianSpec } } : current);
  ...
  return <main className="app-root">
    ...
    {historyOpen && <div className="modal-backdrop"><section className="modal history-modal"><header><div><p className="eyebrow">EXPERIMENT VERSIONS</p><h2>切换与管理实验</h2></div><button onClick={() => setHistoryOpen(false)}>×</button></header><div className="run-list">{runs.map((run) => { const editable = !['campus-32-baseline-v1', 'guardian-human-v1', 'guardian-human-v2'].includes(run.id); return <div key={run.id} className={`run-item ${runData?.run.id === run.id ? 'selected' : ''}`}><button className="run-select" onClick={async () => { await loadRun(run.id); setHistoryOpen(false); }}><span className={`status ${run.status}`}>{statusText[run.status] || run.status}</span><span><strong>{run.name}</strong><small>{fmtDate(run.createdAt, true)} · {run.provider}/{run.model} · {run.densityScale}%</small>{run.notes && <i>{run.notes}</i>}{run.errorSummary && <em>{run.errorSummary}</em>}</span><b>{run.completedCount}/{run.selectedCount}</b></button>{editable && <div className="run-actions">{['running','partial','failed'].includes(run.status) && <button onClick={() => resumeRun(run)}>{run.status === 'running' ? '继续运行' : '修复后重试'}</button>}<button onClick={() => { setEditRun({ id: run.id, name: run.name, notes: run.notes || '' }); setHistoryOpen(false); }}>修改</button><button className="delete-run" onClick={() => deleteExperiment(run)}>删除</button></div>}</div>; })}</div></section></div>}
