...
export default function MemoryLab() {
  ...
  const [config, setConfig] = useState({ name: `模型对话实验`, provider: 'deepseek', model: 'deepseek-chat', baseUrl: '', apiKey: '', historicalDays: 28, densityScale: 50, userCount: 32, concurrency: 2, promptTemplateId: '', promptName: '', userPrompt: '', agentPrompt: '', guardianSpec: '' });
  ...
  async function renameExperiment(run: RunRecord) {
    const name = window.prompt('新的实验名称', run.name)?.trim();
    if (!name || name === run.name) return;
    const response = await fetch('/api/simulator', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'rename_run', runId: run.id, name }) });
    const result = await response.json() as any;
    if (!response.ok) { setToast(result.error || '改名失败'); return; }
